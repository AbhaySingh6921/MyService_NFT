...
export function BuyTicketpop({ onClose ,showToast }) {
  const { buyTicket, contracts, address , notify} = useWeb3(); // 👈 fetch wallet address
...
const remainingTickets = maxTickets - totalSold;
 ...
 useEffect(() => {
  ...
  // Handle buy tickets (smart contract + backend)
  const handleBuy = async () => {
    ...
    amount
      ...
      // if (result.success) {
      //   notify("🎉 Ticket purchased & stored!");
      // } else {
      //   notify("Backend error: " + result.error);
      // }

      onClose();
    } catch (err) {
      console.error(err);
      // notify("❌ Transaction or backend failed.");
    } finally {
      setLoading(false);
    }
};
